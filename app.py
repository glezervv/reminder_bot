...
async def send_welcome(message: types.Message):
    await message.reply(f"Привет! Я бот-напоминалка. Твой ID: {message.from_user.id}\n"
...
async def add_reminder(message: types.Message):
    try:
        ...
        logger.info(
            f"Описание: '{description}', Дата: '{date}', Время: '{time}'")
...
def index():
    try:
        ...
        logger.error(f"Ошибка в веб-форме: {str(e)}")
